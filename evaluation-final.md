...
> - **Agno** — a four-class library API (`AccuracyEval`, `ReliabilityEval`, `PerformanceEval`, `AgentAsJudgeEval`)
> - **Deep Agents** — a pytest + LangSmith trajectory-scoring harness with a two-tier assertion model
> - **Hermes** — a first-principles mental model (outcome → trajectory → step; capability / reliability / safety)
>
> Read top-to-bottom this is a complete, opinionated playbook for evaluating AI agents. Each section notes which source(s) it draws from.

---

## Part 1 — The Core Mental Model

### Why agents need their own evaluation lens *(Hermes)*
...
### The three nested layers *(Hermes)*
...
### The capability / reliability / safety split *(Hermes)*
...
### How the three frameworks frame "is my agent good?"

| Framework | Decomposition | Core abstraction |
|---|---|---|
| **Hermes** | outcome → trajectory → step; capability / reliability / safety | dimensions × methods |
| **Agno** | accuracy / reliability / performance / custom-judge | four eval classes |
| **Deep Agents** | success (correctness) vs. expect (efficiency) | the **trajectory** + two-tier scorer |

**Agno's split** — Agno divides "is my agent good?" into three orthogonal questions plus a flexible fourth:
...
**Deep Agents' one-sentence model** — *An eval is a test that runs a real agent against a real LLM, records what the agent did (its **trajectory**), and asserts on that record — with correctness checks that fail the build and efficiency checks that only get logged.*

---

## Part 2 — Evaluation Dimensions (the "what to measure") *(Hermes)*
...
**Critical distinction — `pass@k` vs `pass^k`** *(Hermes; echoed by Deep Agents' "N trials" and Agno's `num_iterations`)*:
...
### a) Programmatic / verifiable checks *(Hermes, Agno reliability, Deep Agents success-tier)*
...
- **Agno's `ReliabilityEval`** is the canonical packaged form: a fully deterministic check on the tool-call trace, no LLM, designed for CI/pytest.
- **Deep Agents' success assertions** include literal text/file checks: `final_text_contains`, `file_equals`, `file_contains`, etc.

### b) LLM-as-judge *(all three)*
...
- **Deep Agents** wraps [openevals](https://github.com/langchain-ai/openevals) as `llm_judge`: each criterion graded independently, all must pass; criteria run concurrently; can see text-only or the full trajectory.

### c) Human evaluation *(Hermes)*
...
### d) Trajectory / process evaluation *(Hermes, Deep Agents)*
Inspect the *sequence* of actions, not just the end state. Did it call the refund tool before verifying the order? Did it loop? This catches "right answer, dangerous path" cases. Deep Agents reifies this as the **`AgentTrajectory`** abstraction (below).

### e) Simulation / sandbox environments *(Hermes, Deep Agents Harbor)*
Run the agent against a fake-but-faithful environment (mock APIs, seeded DB, simulated user) so you can test destructive actions safely and reproducibly. Deep Agents' **Harbor** extends this to full sandboxed terminal benchmarks (Terminal Bench 2.0 across Docker/Modal/Daytona/etc.).

### f) Performance / cost profiling *(Agno)*
**Agno's `PerformanceEval`** measures (doesn't judge): wraps any callable, runs it `num_iterations` times with optional `warmup_runs`, and profiles wall-clock time + memory via `tracemalloc`. It reports `avg/min/max/std_dev` plus **`median` and `p95`** — the tail-latency focus is deliberate, since averages hide tail latency.

---

## Part 4 — The Central Abstraction: Trajectory → Assertions *(Deep Agents)*

The Deep Agents framework rests on one transformation:
...
### The Two-Tier Insight (the load-bearing design choice) *(Deep Agents)*
...
This maps cleanly onto the others: Agno's `ReliabilityEval.assert_passed()` is a hard gate; its `PerformanceEval` is a logged gauge. Hermes phrases the same idea as **hard gates on safety-critical behaviors, trends/averages elsewhere.**

---

## Part 5 — The Framework Loop (mental workflow) *(Hermes)*
...
### The layered architecture in practice *(Deep Agents)*
...
Each layer answers a different question — core loop: "did this one run pass?"; reporter: "what were aggregate metrics this run?"; CLI/trials: "is this number real or just variance?"; LangSmith/radar: "how do models compare across capabilities?"

### Signal vs. noise: run it N times *(Deep Agents + Agno)*

LLMs are non-deterministic; a single run is an anecdote, N runs is a measurement.
- **Deep Agents** — `evals trials --trials N` aggregates **mean / median / stdev / min / max** per metric, so you can report "0.85 ± 0.02".
- **Agno** — every eval class takes `num_iterations` for statistical stability; results carry `avg_score`, `min/max`, `std_dev`.
...
### Capability fingerprint via categories *(Deep Agents)*
...
### Exit-code semantics for automation *(Deep Agents)*
...
## Part 6 — Worked Example: a Customer-Support Agent *(Hermes, with Agno API mapping)*
...
**Programmatic / process check** — *(Hermes; Agno's `ReliabilityEval`; Deep Agents' success-tier + `tool_call` step pinning)*:
```python
...
**LLM-as-judge** — *(Hermes rubric; Agno's `AgentAsJudgeEval`; Deep Agents' `llm_judge`)*:
```
...
## Part 7 — Reliability & Safety: the part most teams skip *(Hermes)*

- **Run each task k times** (different seeds/temperature). Report `pass^k`, not one lucky run. (Agno `num_iterations`, Deep Agents `--trials N`.)
...
- **Drift monitoring:** keep running the eval suite on a schedule in production — providers update models under you, and tools change. (Agno supports this via live post-hooks; Deep Agents via scheduled CI workflows.)

---

## Part 8 — Concrete API / Tooling Cheat-Sheets

### Agno — four eval classes *(library API)*
...
| Layer | Agno eval | Runs in | Catches |
|-------|-----------|---------|---------|
...
Shared Agno conventions: sync `.run()` + async `.arun()`; `num_iterations` for stability; `db=...` to persist & query runs; `print_results`/`print_summary` rich tables; `file_path_to_save_results` to disk. All four work on both `Agent` and `Team`.

### Deep Agents — write an eval as a test *(harness API)*
...
- **Integrated academic benchmarks:** FRAMES (multi-hop retrieval), Nexus (nested fn composition), BFCL v3 (stateful multi-turn tool calling), tau2-bench airline (multi-turn scored on DB state), MemoryAgentBench (long-context memory). **Harbor** adds Terminal Bench 2.0 in sandboxes.
- **Requires** `LANGSMITH_TRACING=true` + `LANGSMITH_API_KEY` + a provider key matching `--model`.
...
## Part 9 — Anti-patterns to avoid *(Hermes)*
...
## Part 10 — Quick-Reference Scorecard Template *(Hermes)*
...
1. **Grade the trajectory, not just the output** — agents *act*, so evaluate the actions (Deep Agents' `AgentTrajectory`; Hermes' three layers).
2. **Two tiers / two metric kinds:** correctness *gates* (hard-fail), efficiency *informs* (logged). Reserve **hard gates** for safety-critical behaviors (Deep Agents `.success`/`.expect`; Agno `assert_passed` vs perf; Hermes hard gates).
3. **Pick the cheapest sufficient method:** programmatic > calibrated LLM-judge > human spot-check; profile cost/latency separately (Agno's four classes map onto this directly).
